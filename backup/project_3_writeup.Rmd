...
While I didn't produce animations for the predicted movement, this would be relatively easy to do (or _could_ be easy if I didn't hard-code so much). In a more complete analysis, animations for predicted movement would be a nice accompaniment to the model results. In the same vein, integrating elements of the synthetic population from [Project 2](https://emiliolr.github.io/abm/project_2_writeup.html#producing-the-synthetic-population) could allow for interesting simulations that might giive us insight into shifting demographics across Eswatini as agents make long-term movement decisions. Unfortunately, I didn't quite get to the point where I would have brought in the results from Project 2, but it's still interesting to think a about what could've been accomplished if I had had more time!
...
- [Following along with the vignette]()
