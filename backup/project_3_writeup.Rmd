...
First, the coding scheme for the districts (which I will stick to throughout this write-up) is: 1 - Hhohho; 2 - Lubombo; 3 - Manzini; 4 - Shiselweni. In this matrix (and in all matrices in this deliverable), rows indicate the origin node and columns indicate the destination node. For example, in the OD matrix above, the cell corresponding to row 3 and column 4 shows the flows from Manzini to Shiselweni (about `r real_OD_matrix[3, 4] %>% floor()` people). There are zeroes along the diagonal as we are interested only in the flows _between_ districts. I have included a plot showing the coding scheme below, along with the centroid locations for each district.
...
In addition to the OD matrix, I created a matrix that included the distances in meters between all district _centroids_, which is generally the standard procedure for calculating distance between region when inputting into a gravity model (more on this later).
...
ggplot() +
  ...
  geom_sf(data = swz_mk, fill = "khaki", color = "black", size = 0.8) +
